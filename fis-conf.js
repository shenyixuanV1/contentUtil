<!--param start-->
//修改cdn的绝对路径（测试环境）
fis.set('cdn-path', 'http://test.nie.163.com/test_cdn/nie-js/nie-contentUtil');
//修改cdn的绝对路径（正式环境）
fis.set('cdn-path-release', 'https://nie.res.netease.com/comm/js/nie/util/');
...
fis.match(/^\/src\/css\/(.*)\.(css|less)$/i,{
    release : 'alertBox/css/$1',
    useHash: false
},true);
//配置打本地包
...
fis.media('dist')
	...
	.match('**', {
		...
		deploy: [fis.plugin('encoding'), fis.plugin('local-supply', {
			to: './dist',
			exclude: ['inline', 'temp_file', 'config', '_reset.css']
		})]
	})
